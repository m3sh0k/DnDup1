...
bool mapviewwidget::eventFilter(QObject *object, QEvent *event)
{
    if (object == ui->graphicsView->viewport()) { // Проверяем, относится ли событие к viewport
        switch (event->type()) { // Определяем тип события
        case QEvent::MouseButtonPress: { // Если событие - нажатие кнопки мыши
            QMouseEvent *mouseEvent = static_cast<QMouseEvent *>(event); // Преобразуем событие к QMouseEvent
            if (mouseEvent->button() == Qt::RightButton) { // Если нажата правая кнопка мыши
                createContextMenu(mouseEvent->pos()); // Вызываем метод для отображения контекстного меню
                return true; // Блокируем стандартное поведение
            }
            break;
        }
        case QEvent::MouseButtonRelease: // Обработка отпускания кнопки мыши
            handleDrag(static_cast<QMouseEvent *>(event)); // Обрабатываем перетаскивание
            break;
        case QEvent::Wheel: // Обработка прокрутки колеса мыши
            handleZoom(static_cast<QWheelEvent *>(event)); // Обрабатываем масштабирование
            return true; // Блокируем стандартное поведение прокрутки
        default:
            break; // Игнорируем остальные события
        }
    }
    return QWidget::eventFilter(object, event); // Вызываем стандартный обработчик событий
}
